...
; Binning parameters
give_map_k, pars.res_pix, dblarr(pars.nx_large,pars.ny_large), k_map, k_mapx ; k [rad-1]
k_map  = k_map  * 2.0d0*!dpi ;l
wk     = where( k_map ne 0)

; Bins
kmin  = min( k_map[wk]) ;; on the "large" map, l
...
if keyword_set(bintab_in) then begin
   ...
   k_range   = [kmin, pars.k_nyquist] ;l
   make_bintab, k_range, pars.dk_min, bintab, /float, delta_l_over_l=pars.delta_l_over_l, log=pars.log_binning ;l
...
if bypass_mtt_bb eq 1 then begin
   ...
   if (!arch eq "franklin") or (!arch eq "parallel") then begin
      ...
      print, " PARRALLELE "
      spawn, "mpirun /home/mvancuyck/bin/poker_mbb_mpi "+parfile
      print, "after"
   endif else begin
