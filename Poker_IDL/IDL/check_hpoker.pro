...
index_tt             = -3       ; to start easy, then try with -3
...
;; Build common bintab now
print, "arrays1.bintab: ", arrays1.bintab
print, "arrays2.bintab: ", arrays2.bintab

bintab1 = [0.0000000,  78.310548,  273.81310,  468.52464, $
           663.23618, 2190.5048,  3748.1971, 5062.500]
bintab2 = [0.0d0, $
           663.23618, 2190.5048,  3748.1971, 5062.500, $
           6863.5818,  8421.2741,  9978.9664,  11536.659,  13094.351,  $
           14652.043,  16209.736,  17830.709,  19613.780,  21575.158,  23732.674,  26105.941, $
           28716.536,  31588.189,  34747.008,  40500.000]

;; Re-init arrays with these bintabs (and compute large scale power spetrum)
ipoker, map1, res_arcmin*8, k1, pk1, mask=mask_apod, $
        beta=beta, remove=remove_1st_bin, out_params=params1, $
        out_arrays=arrays1, nx_large=nx1_large, ny_large=ny1_large, $
        bintab=bintab1
...
;; Make simulations to compute the transfer function, the covariances
;; etc...
nk1 = n_elements(k1)
nk2 = n_elements(k2)
map_temp = dblarr(nx1,ny1)
map2     = dblarr(nx2,ny2)
pk_res   = dblarr(nmc, nk1+8*8*nk2)
for imc=0, nmc-1 do begin
   if imc mod 10 eq 0 then print, imc
   ;; simulate the full map
   cls2map, [0,1], [0,1], nx_large, ny_large, res_arcmin, map_junk, cu_t, /no_cu_t_reset
   ;; degrade resolution for large scales
   for i1=0, nx1-1 do begin
      for j1=0, ny1-1 do begin
         map_temp[ i1,j1] = avg( map[ i1*8:(i1+1)*8-1, j1*8:(j1+1)*8-1])
      endfor
   endfor
   ipoker, map_temp, res_arcmin/8, k1, pk1, $
           in_params=params1, in_arrays=arrays1
   pk_res[imc,0:nk1-1] = pk1
   
   ;; small scales
   p=0
   for i=0,7 do begin
      for j=0,7 do begin
         ipoker, map_junk[i*nx2:(i+1)*nx2-1,j*ny2:(j+1)*ny2-1], res_arcmin, k2, pk2, $
                 in_arrays=arrays2, in_params=params2
         pk_res[imc, nk1+p*nk2:nk1+(p+1)*nk2-1] = pk2
         p++
      endfor
   endfor
endfor
...
;; Covariance matrix
mc_reduce, pk_res, pk_avg, sigma_pk_avg, cov_mat, xcorr

imr_log = [-70,-40]
wind, 1, 1, /free, /large
my_multiplot, 2, 2, pp, pp1, /rev
imview, cov_mat, position=pp1[0,*], title='Cov mat (lin)'
imview, alog( abs(cov_mat)), position=pp1[1,*], title='Cov mat log', /noerase, imr=imr_log
imview, xcorr, position=pp1[2,*], title='xcorr', /noerase
my_multiplot, /reset
stop
