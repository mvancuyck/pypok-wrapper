...
poker_make_mask, nx1, ny1, 1, 0, 1, mask_apod1, apod_length=apod_length

;; Compute power spectrum and initialize arrays for the large map
ipoker, map1, res_arcmin/8, k1, pk1, mask=mask_apod1*mask1, $
...
nx2_large = 128
ny2_large = 128
poker_make_mask, nx2, ny2, 1, 0, 1, mask_apod2, apod_length=apod_length
ipoker, map[0:nx2-1,0:ny2-1], res_arcmin, k2, pk2, mask=mask_apod2*mask[0:nx2-1,0:ny2-1], $
        beta=beta, remove=remove_1st_bin, out_params=params2_0, $
        out_arrays=arrays2_0, nx_large=nx2_large, ny_large=ny2_large
...
for i=0,7 do begin
   for j=0,7 do begin
      ...
      ipoker, map[i*nx2:(i+1)*nx2-1,j*ny2:(j+1)*ny2-1], res_arcmin, k2, pk2, $
              mask=mask_apod2*mask[i*nx2:(i+1)*nx2-1,j*ny2:(j+1)*ny2-1], $
              beta=beta, remove=remove_1st_bin, out_params=params, $
              out_arrays=arrays, nx_large=nx2_large, ny_large=ny2_large
      nick = strtrim(i,2)+"_"+strtrim(j,2)
      junk = execute( "params2_"+nick+" = params")
      junk = execute( "arrays2_"+nick+" = arrays")
   endfor
endfor
