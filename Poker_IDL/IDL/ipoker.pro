...
out_params.nx      = nx
out_params.ny      = ny
out_params.res_pix = res_arcmin*!arcmin2rad ;rad

out_params.k_nyquist = !dpi/out_params.res_pix ; l
...
if out_params.keep_avg eq 0 then data_map = map - mean(map[where( out_arrays.mask gt 0.d0)]) else data_map = map ; remove mean, MJy/sr
...
;       masked map [MJy/sr]             rad                ??       #map1 = map2

;poker_writefits, "idl_arr/cmn.fits" , c_mn_t, /silent
cmn2cb, out_arrays.map_b, out_arrays.map_k_binning*c_mn_t, pb_mes, sigma_pb_mes ;MJy**2/sr
...
;; !!!!!!!!! SIDES Units handling
pb_out = pb_out * 1e12
sigma_pk_out = sigma_pk_out * 1e12
